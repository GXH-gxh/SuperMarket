    ...
    public void exportGoods(OutputStream outputStream) {
        ...
        for (GoodsVo gv : list) {
           ...
           /* contentRow.createCell(3).setCellValue(gv.getMemberPrice());*/
            // 批发价
            contentRow.createCell(3).setCellValue(gv.getWholePrice());
            // 进货价
            contentRow.createCell(4).setCellValue(gv.getCostPrice());
            if (gv.getGoodsCount() == null){
                contentRow.createCell(5).setCellValue("");
            }else {
                // 库车
                contentRow.createCell(5).setCellValue(gv.getGoodsCount());
            }
            // 商品类型
            contentRow.createCell(6).setCellValue(gv.getGoodsTypeName());
            // 单位
            contentRow.createCell(7).setCellValue(gv.getUnitId());
